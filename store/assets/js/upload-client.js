class HentHubUploader {
    ...
    async fetchStoreManifest() {
        try {
            // UI is already in /store, so relative path is manifests/
            const url = CONFIG.mode === 'GITHUB' 
                ? 'manifests/store-manifest.json'
                : `${CONFIG.localUrl}/store/manifests/store-manifest.json`;
        ...
        if (!app.entryPoint && !app.widgetClass) {
            ...
            try {
                ...
                const url = CONFIG.mode === 'GITHUB'
                    ...
                    : `${CONFIG.localUrl}/store/manifests/${subfolder}/${appId.toLowerCase()}.json`;
    ...
    async uploadToLocal(pkgPath, pkgContent, iconPath, iconContent, screenshots, metadata) {
        ...
        if (pkgContent) await upload('store/' + pkgPath, pkgContent);
        if (iconContent) await upload('store/' + iconPath, iconContent);
        ...
        const slimEntry = {
            ...
            downloadUrl: `${CONFIG.localUrl}/store/${pkgPath}`,
            iconUrl: `${CONFIG.localUrl}/store/${iconPath}`,
